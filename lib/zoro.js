...
function () {
  ...
  _createClass(Zoro, [{
    ...
    value: function setup() {
      (0, _util.assert)(!!this.store, 'the setup function must be call after start(false)');

      if (!this._isSetup) {
        this.setupModel(this.models);
        this.handleSetup.apply(undefined, [{
          put: (0, _util.putCreator)(this.store),
          select: (0, _util.selectCreator)(this.store)
        }]);
        this.plugin.emit(_constant.PLUGIN_EVENT.ON_SETUP, this.store);
      }
