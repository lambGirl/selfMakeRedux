...
var _createReducer2 = _interopRequireDefault(require("src/lib/createReducer"));
...
var assertOpts = function assertOpts(opts) {
  ...
  (0, _until.assert)(!!namespace, "the model's namespace is necessary, but we get ".concat(namespace));
};
...
function () {
  function Model(opts) {
    ...
    var namespace = opts.namespace,
        ...
        setup = _opts$setup === void 0 ? _until.noop : _opts$setup; //将当model传入的对象进行抽离,封装,拓展
  ...
  _createClass(Model, [{
    ...
    value: function createReducer(reducers) {
      ...
      var _reducers = Object.keys(reducers).reduce(function (combine, key) {
        ...
        (0, _until.assert)((0, _until.isFunction)(reducer), "the reducer must be an function, but we get ".concat(_typeof(reducer), " with type ").concat(type));
    ...
    value: function createActions(actions) {
      ...
      return Object.key(actions).reduce(function (combine, name) {
    ...
    value: function createEffects(effects) {
      ...
      return Object.keys(effects).reduce(function (combine, key) {
        ...
        (0, _until.assert)((0, _until.isFunction)(effect), "the effect must be an function, but we get ".concat(_typeof(effect), " with type ").concat(type));
